...
def candidate_quality(s,book):
    ...
    if book=='CORE':
        eligible=call=='STRONG BUY' and diff>=7 and rvol>=1.0 and r24<20
        score=diff*2+rvol+max(0,-abs(r4)/10)
        reasons=[] if eligible else ['CORE SELECTIVITY']
    else:
        eligible=call in {'BUY','STRONG BUY','SELL','STRONG SELL'} and diff>=6 and rvol>=1.15
        score=diff*2+rvol*2+abs(r4)
        reasons=[] if eligible else ['SWING EDGE INSUFFICIENT']
    return eligible,score,reasons

def update_wallet(wallet,signals,risk_map,book):
    ...
    ranked=[]
    for s in signals:
        ok,score,reasons=candidate_quality(s,book)
        if ok: ranked.append((score,s))
    ...
    for score,s in ranked:
        ...
        p={'position_id':f'{book}_{sym}_{str(s.get("recorded_at") or now()).replace(":","")}','book':book,'symbol':sym,'name':s.get('name') or sym,'narrative':s.get('narrative') or '','direction':d,'signal':s.get('signal'),'entry_time':s.get('recorded_at') or now(),'entry_price':entry,'current_price':entry,'allocated_cash':alloc,'units':alloc/entry,'status':'OPEN','unrealised_return':-0.30,'unrealised_pnl':-alloc*0.003,'maximum_favourable_excursion_pct':0.0,'maximum_adverse_excursion_pct':-0.30}
