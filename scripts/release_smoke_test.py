...
def test_page_navigation() -> None:
    ...
    radio_match = re.search(r'st\.sidebar\.radio\("Navigation",\[(.*?)\],label_visibility', text, re.S)
    if not radio_match:
    ...
    handlers = {"Today"}
    handlers.update(re.findall(r'(?:if|elif) selection=="([^"]+)"', text))
    # Signal Lab is the final fallback `else` page in this application.
